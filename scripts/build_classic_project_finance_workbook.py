...
def set_font(cell, color=BLACK, bold=False, size=9):
...
def project_formula(sheet, value_col, key_col='A', key='$B$3', first=2, last=1000):
...
def build_dashboard(wb):
    ...
    dv.add(ws['B3'])
    label(ws['D3'], 'Base Year')
    hardcode(ws['E3'], 2026, fmt='0', align='center')
    label(ws['G3'], 'Units')
    ws['H3'] = 'VND bn / x / %'
    style_cell(ws['H3'], bg=WHITE, bold=True, border=True)

    section(ws, 5, 'KEY MODEL OUTPUTS', 14)
    metrics = [
        ('A6','B6','Installed Capacity (MWp)', project_formula('18_Portfolio','D'), FMT_NUM),
        ...
        ('G6','H6','Standalone Debt (VND bn)', project_formula('18_Portfolio','E'), FMT_BN),
        ('J6','K6','Minimum DSCR', project_formula('14_Coverage','B'), FMT_MULT),
        ('A8','B8','LLCR', project_formula('14_Coverage','C'), FMT_MULT),
        ('D8','E8','PLCR', project_formula('14_Coverage','D'), FMT_MULT),
        ('G8','H8','Project NPV (VND bn)', '=' + project_formula('15_Returns_Discount','E')[1:] + '/1000000000', FMT_BN),
        ('J8','K8','Equity NPV (VND bn)', '=' + project_formula('15_Returns_Discount','D')[1:] + '/1000000000', FMT_BN),
    ]
    for lc, vc, txt, fx, fmt in metrics:
        label(ws[lc], txt)
        formula(ws[vc], fx, fmt, bg=CYAN, bold=True)
    label(ws['M6'], 'Sponsor Status')
    formula(ws['N6'], project_formula('19_IC_Bankability','B'), align='center', bold=True)
    label(ws['M8'], 'Lender Status')
    formula(ws['N8'], project_formula('19_IC_Bankability','C'), align='center', bold=True)
    add_status_rules(ws, 'N6', 'N6')
    add_status_rules(ws, 'N8', 'N8')

    section(ws, 11, 'FINANCING & CREDIT METRICS', 14)
    debt = [
        ('A12','B12','All-in Debt Rate', project_formula('11_Debt_Terms','G',key_col='B'), FMT_PCT2),
        ('D12','E12','Leverage Cap', project_formula('11_Debt_Terms','M',key_col='B'), FMT_PCT),
        ('G12','H12','Debt Tenor', project_formula('11_Debt_Terms','N',key_col='B'), '0'),
        ('J12','K12','Covenant DSCR', project_formula('11_Debt_Terms','J',key_col='B'), FMT_MULT),
        ('A14','B14','Lock-up DSCR', project_formula('11_Debt_Terms','K',key_col='B'), FMT_MULT),
        ('D14','E14','DSRA Months', project_formula('11_Debt_Terms','O',key_col='B'), '0'),
        ('G14','H14','DSCR Headroom', '=K6-K12', FMT_MULT),
        ('J14','K14','Coverage Status', project_formula('14_Coverage','J'), '@'),
    ]
    for lc, vc, txt, fx, fmt in debt:
        label(ws[lc], txt)
        formula(ws[vc], fx, fmt, bg=YELLOW if vc in {'B12','E12','H12','K12','B14','E14'} else CYAN, bold=True)
    ...
    for i,h in enumerate(headers,1):
        style_cell(ws.cell(18,i,h), bg=ORANGE, bold=True, border=True, align='center')
    ...
    for idx,sid in enumerate(scenario_rows,19):
        ws.cell(idx,1,sid)
        style_cell(ws.cell(idx,1), bg=LIGHT_BLUE, color=LINK_GREEN, bold=True, border=True)
        for col, src in [(2,'C'),(3,'D'),(4,'F'),(5,'P')]:
            fmt = FMT_PCT if col in (2,3) else FMT_PCT2 if col==4 else FMT_MULT
            formula(ws.cell(idx,col), f'=INDEX(\'17_Scenarios_Sensitivity\'!${src}$2:${src}$14,MATCH($A{idx},\'17_Scenarios_Sensitivity\'!$A$2:$A$14,0))', fmt, bg=YELLOW if col<5 else CYAN, bold=col==5)
        ws.cell(idx,6, f'=IF(E{idx}<1.20,"BREACH","PASS")')
        style_cell(ws.cell(idx,6), bg=WHITE, bold=True, border=True, align='center')
        add_status_rules(ws, f'F{idx}', f'F{idx}')
    ...
    decision = [
        ('A35','B35','Binding Issue', project_formula('19_IC_Bankability','D')),
        ('A37','B37','Recommended Action', project_formula('19_IC_Bankability','E')),
        ('A39','B39','Final Classification', project_formula('19_IC_Bankability','I')),
    ]
    for lc,vc,txt,fx in decision:
        label(ws[lc],txt)
        ws.merge_cells(start_row=ws[vc].row,start_column=2,end_row=ws[vc].row,end_column=14)
        formula(ws[vc],fx,align='left',bold=True)
    add_status_rules(ws, 'B39:N39', 'B39')
    for col,w in {'A':36,'B':16,'C':3,'D':24,'E':16,'F':16,'G':24,'H':16,'I':3,'J':24,'K':16,'L':3,'M':24,'N':18}.items():
...
def build_model_log(wb):
    ...
    for i,h in enumerate(headers,1):
        style_cell(ws.cell(5,i,h),bg=ORANGE,bold=True,border=True,align='center')
    rows=[
    ...
    for r,row in enumerate(rows,6):
        for c,v in enumerate(row,1):
            ws.cell(r,c,v)
            style_cell(ws.cell(r,c),bg=GREEN if r==6 else WHITE,bold=(r==6),border=True,wrap=True)
    for col,w in {'A':16,'B':12,'C':16,'D':64,'E':30,'F':18}.items():
        ws.column_dimensions[col].width=w
    configure(ws,'A5')
...
def build_input_scenarios(wb):
    ...
    label(ws['A4'],'Selected Project')
    formula(ws['B4'],"='01_Dashboard'!$B$3",align='center',bold=True)
    ...
    for i,s in enumerate(scenarios,8):
        ws.cell(7,i,s)
        style_cell(ws.cell(7,i),bg=NAVY,color=WHITE,bold=True,border=True,align='center',wrap=True)
    ...
    for rr,(name,col,fmt) in enumerate(rows,9):
        ...
        for i in range(8,16):
            formula(ws.cell(rr,i),f'=INDEX(\'17_Scenarios_Sensitivity\'!${col}$2:${col}$14,MATCH({ws.cell(7,i).coordinate},\'17_Scenarios_Sensitivity\'!$A$2:$A$14,0))',fmt,bg=YELLOW)
    ...
    for r,(name,col,fmt) in enumerate(finance,22):
        label(ws.cell(r,1),name)
        formula(ws.cell(r,2),project_formula('11_Debt_Terms',col,key_col='B'),fmt,bg=YELLOW,bold=True)
...
def build_construction(wb):
    ...
    ws['A7']='Construction Month'
    style_cell(ws['A7'],bg=PURPLE,color=WHITE,bold=True,border=True)
    for i in range(12):
        c=8+i
        ws.cell(7,c,i+1)
        style_cell(ws.cell(7,c),bg=PURPLE,color=WHITE,bold=True,border=True,align='right')
    ...
    for rr,(name,col,fmt,bg) in enumerate(rows,9):
        ...
        for i in range(12):
            cc=8+i
            if fmt==FMT_BN:
                fx=f'=SUMIFS(\'07_CAPEX_Construction\'!${col}:${col},\'07_CAPEX_Construction\'!$A:$A,$B$4,\'07_CAPEX_Construction\'!$B:$B,{i+1})/1000000000'
            else:
                fx=f'=SUMIFS(\'07_CAPEX_Construction\'!${col}:${col},\'07_CAPEX_Construction\'!$A:$A,$B$4,\'07_CAPEX_Construction\'!$B:$B,{i+1})'
            formula(ws.cell(rr,cc),fx,fmt,bg=bg,bold=name.startswith('Cumulative'))
    section(ws,18,'SOURCES & USES SUMMARY',20)
    summary=[
        ('Gross Construction CAPEX','=' + project_formula('07_CAPEX_Construction','J')[1:] + '/1000000000'),
        ('Net Construction CAPEX','=' + project_formula('07_CAPEX_Construction','K')[1:] + '/1000000000'),
        ('VAT','=' + project_formula('07_CAPEX_Construction','L')[1:] + '/1000000000'),
        ('Interest During Construction (IDC)',"=SUM(H13:S13)"),
        ('Total Uses','=' + project_formula('07_CAPEX_Construction','N')[1:] + '/1000000000'),
    ]
    for r,(name,fx) in enumerate(summary,19):
        label(ws.cell(r,1),name)
        formula(ws.cell(r,5),fx,FMT_BN,bg=CYAN if r==23 else WHITE,bold=r==23)
...
def build_project_workings(wb):
    ...
    for rr,(name,col,fmt,bg) in enumerate(rows,10):
        ...
        for i in range(16):
            cc=8+i
            if name=='EBITDA':
                fx=f'={ws.cell(rr-2,cc).coordinate}-{ws.cell(rr-1,cc).coordinate}'
            else:
                fx=f'=SUMIFS(\'09_Project_CF_CFADS\'!${col}:${col},\'09_Project_CF_CFADS\'!$A:$A,$B$4,\'09_Project_CF_CFADS\'!$B:$B,{i})/1000000000'
            formula(ws.cell(rr,cc),fx,fmt,bg=bg,bold=name in {'EBITDA','CFADS'})
    for r in [12,20]:
        for c in range(1,24):
            ws.cell(r,c).border=Border(top=MEDIUM,bottom=THIN)
    for c in range(1,24):
        ws.cell(20,c).border=Border(top=MEDIUM,bottom=DOUBLE)
...
def build_financing(wb):
    ...
    terms=[
        ('Standalone Debt Capacity',project_formula('18_Portfolio','E'),FMT_BN),
        ('Reference Rate',project_formula('11_Debt_Terms','E',key_col='B'),FMT_PCT2),
        ('Credit Spread',project_formula('11_Debt_Terms','F',key_col='B'),'0 "bps"'),
        ('All-in Debt Rate',project_formula('11_Debt_Terms','G',key_col='B'),FMT_PCT2),
        ('Target DSCR',project_formula('11_Debt_Terms','I',key_col='B'),FMT_MULT),
        ('Covenant DSCR',project_formula('11_Debt_Terms','J',key_col='B'),FMT_MULT),
        ('Lock-up DSCR',project_formula('11_Debt_Terms','K',key_col='B'),FMT_MULT),
        ('Debt Tenor',project_formula('11_Debt_Terms','N',key_col='B'),'0'),
        ('DSRA Months',project_formula('11_Debt_Terms','O',key_col='B'),'0'),
    ]
    for r,(name,fx,fmt) in enumerate(terms,7):
        label(ws.cell(r,1),name)
        formula(ws.cell(r,5),fx,fmt,bg=YELLOW,bold=True)
    section(ws,18,'SENIOR DEBT REPAYMENT SCHEDULE',18)
    ws['A19']='Year Number'
    style_cell(ws['A19'],bg=PURPLE,color=WHITE,bold=True,border=True)
    for i in range(10):
        c=8+i
        ws.cell(19,c,i+1)
        style_cell(ws.cell(19,c),bg=PURPLE,color=WHITE,bold=True,border=True,align='right')
    ...
    for r,name in enumerate(names,21):
        label(ws.cell(r,1),name)
    for i in range(10):
        c=8+i
        col=ws.cell(1,c).column_letter
        pcol=ws.cell(1,9+i).column_letter
        ...
        formula(ws.cell(29,c),f'=IF({col}27<$E$12,"BREACH","PASS")',bg=WHITE,color=BLACK,bold=True,align='center')
        add_status_rules(ws,f'{col}29',f'{col}29')
    ...
    for r,(name,fx,fmt) in enumerate(checks,33):
        label(ws.cell(r,1),name)
        formula(ws.cell(r,5),fx,fmt,bg=CYAN if r in (35,37) else WHITE,bold=True)
    add_status_rules(ws,'E37','E37')
    ws.column_dimensions['A'].width=36
    for c in range(8,18):
        ws.column_dimensions[ws.cell(1,c).column_letter].width=12
    chart=LineChart()
    chart.title='Senior Debt Amortisation'
    chart.y_axis.title='VND bn'
    chart.x_axis.title='Year'
    chart.add_data(Reference(ws,min_col=8,max_col=17,min_row=26,max_row=26),from_rows=True,titles_from_data=False)
    chart.set_categories(Reference(ws,min_col=8,max_col=17,min_row=19,max_row=19))
    chart.height=7
    chart.width=14
...
def build_waterfall(wb):
    ...
    for i in range(15):
        ...
        ws.cell(7,c,i+1)
        style_cell(ws.cell(7,c),bg=PURPLE,color=WHITE,bold=True,border=True,align='right')
    names=['CFADS','Senior Debt Service','Cash Available Before Reserves','DSRA Opening Balance','DSRA Target','DSRA Funding','DSRA Release','DSRA Closing Balance','Lock-up DSCR','Cash Trap','Equity Distribution','Waterfall Check']
    rowmap={name:9+i for i,name in enumerate(names)}
    for name,r in rowmap.items():
        label(ws.cell(r,1),name)
    for i in range(15):
        c=8+i
        col=ws.cell(1,c).column_letter
        ...
        if i<10:
            fcol=ws.cell(1,8+i).column_letter
            formula(ws.cell(rowmap['Senior Debt Service'],c),f"='06_Financing_Workings'!{fcol}24",FMT_BN)
        else:
            ws.cell(rowmap['Senior Debt Service'],c,0)
            style_cell(ws.cell(rowmap['Senior Debt Service'],c),bg=WHITE,fmt=FMT_BN,border=True,align='right')
        ...
        if i==0:
            ws.cell(rowmap['DSRA Opening Balance'],c,0)
            style_cell(ws.cell(rowmap['DSRA Opening Balance'],c),bg=GREY,fmt=FMT_BN,border=True,align='right')
        else:
            formula(ws.cell(rowmap['DSRA Opening Balance'],c),f'={ws.cell(rowmap["DSRA Closing Balance"],c-1).coordinate}',FMT_BN,bg=GREY,color=BLACK)
        if i<10:
            formula(ws.cell(rowmap['DSRA Target'],c),f'={col}{rowmap["Senior Debt Service"]}*\'06_Financing_Workings\'!$E$15/12',FMT_BN,bg=GREY,color=BLACK)
        else:
            ws.cell(rowmap['DSRA Target'],c,0)
            style_cell(ws.cell(rowmap['DSRA Target'],c),bg=GREY,fmt=FMT_BN,border=True,align='right')
        ...
        if i==9:
            formula(ws.cell(rowmap['DSRA Release'],c),f'={col}{rowmap["DSRA Opening Balance"]}+{col}{rowmap["DSRA Funding"]}',FMT_BN,bg=GREY,color=BLACK)
        else:
            ws.cell(rowmap['DSRA Release'],c,0)
            style_cell(ws.cell(rowmap['DSRA Release'],c),bg=GREY,fmt=FMT_BN,border=True,align='right')
        ...
        if i<10:
            fcol=ws.cell(1,8+i).column_letter
            formula(ws.cell(rowmap['Cash Trap'],c),f'=IF(\'06_Financing_Workings\'!{fcol}27<{col}{rowmap["Lock-up DSCR"]},MAX(0,{col}{rowmap["Cash Available Before Reserves"]}),0)',FMT_BN,bg=YELLOW,color=BLACK)
        else:
            ws.cell(rowmap['Cash Trap'],c,0)
            style_cell(ws.cell(rowmap['Cash Trap'],c),bg=YELLOW,fmt=FMT_BN,border=True,align='right')
...
def build_coverage_returns(wb):
    ...
    metrics=[
        ('Minimum DSCR',project_formula('14_Coverage','B'),FMT_MULT),
        ('Covenant DSCR',project_formula('11_Debt_Terms','J',key_col='B'),FMT_MULT),
        ('DSCR Headroom','=B7-B8',FMT_MULT),
        ('LLCR',project_formula('14_Coverage','C'),FMT_MULT),
        ('PLCR',project_formula('14_Coverage','D'),FMT_MULT),
        ('DSRA Target (VND bn)','=' + project_formula('14_Coverage','E')[1:] + '/1000000000',FMT_BN),
    ]
    for r,(name,fx,fmt) in enumerate(metrics,7):
        label(ws.cell(r,1),name)
        formula(ws.cell(r,2),fx,fmt,bg=CYAN,bold=True)
    ...
    returns=[
        ('Project Cost (VND bn)',"='01_Dashboard'!$E$6",FMT_BN),
        ('Equity Required (VND bn)','=' + project_formula('15_Returns_Discount','B')[1:] + '/1000000000',FMT_BN),
        ('Discount Rate',project_formula('15_Returns_Discount','C'),FMT_PCT2),
        ('Project NPV (VND bn)','=' + project_formula('15_Returns_Discount','E')[1:] + '/1000000000',FMT_BN),
        ('Equity NPV (VND bn)','=' + project_formula('15_Returns_Discount','D')[1:] + '/1000000000',FMT_BN),
    ]
    for r,(name,fx,fmt) in enumerate(returns,16):
        label(ws.cell(r,1),name)
        formula(ws.cell(r,2),fx,fmt,bg=YELLOW,bold=True)
    ws['A23']='Year'
    ws['A24']='Project Cash Flow'
    ws['A25']='Equity Cash Flow'
    for r in (23,24,25):
        style_cell(ws.cell(r,1),bg=PURPLE,color=WHITE,bold=True,border=True)
    for i in range(16):
        c=8+i
        ws.cell(23,c,i)
        style_cell(ws.cell(23,c),bg=PURPLE,color=WHITE,bold=True,border=True,align='right')
        if i==0:
            formula(ws.cell(24,c),'=-$B$16',FMT_BN,bg=WHITE,color=BLACK)
            formula(ws.cell(25,c),'=-$B$17',FMT_BN,bg=WHITE,color=BLACK)
        else:
            pcol=ws.cell(1,8+i).column_letter
            wcol=ws.cell(1,7+i).column_letter
            formula(ws.cell(24,c),f"='05_Project_Workings'!{pcol}20",FMT_BN)
            formula(ws.cell(25,c),f"='07_Cashflow_Waterfall'!{wcol}19",FMT_BN)
    label(ws['A28'],'Project IRR',bg=CYAN)
    formula(ws['B28'],'=IFERROR(IRR(H24:W24),"n.m.")',FMT_PCT2,bg=CYAN,color=BLACK,bold=True)
    label(ws['A29'],'Equity IRR',bg=CYAN)
    formula(ws['B29'],'=IFERROR(IRR(H25:W25),"n.m.")',FMT_PCT2,bg=CYAN,color=BLACK,bold=True)
    ws['D7']='Year'
    ws['E7']='Actual DSCR'
    ws['F7']='Covenant'
    for c in range(4,7):
        style_cell(ws.cell(7,c),bg=ORANGE,bold=True,border=True,align='center')
    for i in range(10):
        r=8+i
        ws.cell(r,4,i+1)
        formula(ws.cell(r,5),f"='06_Financing_Workings'!{ws.cell(1,8+i).column_letter}27",FMT_MULT)
        formula(ws.cell(r,6),'=$B$8',FMT_MULT)
    chart=LineChart()
    chart.title='Annual DSCR vs Covenant'
    chart.add_data(Reference(ws,min_col=5,max_col=6,min_row=7,max_row=17),titles_from_data=True)
    chart.set_categories(Reference(ws,min_col=4,min_row=8,max_row=17))
    chart.height=7
    chart.width=14
...
def build_sensitivity(wb):
    ...
    label(ws['A4'],'Discount Rate')
    formula(ws['B4'],"='08_Coverage_Returns'!$B$18",FMT_PCT2,bg=YELLOW,bold=True)
    section(ws,6,'TWO-DIMENSIONAL PROJECT NPV SENSITIVITY',22)
    shocks=[-0.15,-0.10,-0.05,0,0.05,0.10,0.15]
    ws['G8']='CAPEX \\ CFADS'
    style_cell(ws['G8'],bg=YELLOW,color=NAVY,bold=True,border=True,align='center')
    for i,x in enumerate(shocks,8):
        ws.cell(8,i,x)
        style_cell(ws.cell(8,i),bg=YELLOW,color=NAVY,bold=True,fmt=FMT_PCT,border=True,align='center')
    for i,x in enumerate(shocks,9):
        ws.cell(i,7,x)
        style_cell(ws.cell(i,7),bg=YELLOW,color=NAVY,bold=True,fmt=FMT_PCT,border=True)
        for j in range(8,15):
    ...
    headers=['Scenario','CFADS Factor','CAPEX Factor','Interest Rate','Minimum DSCR','Credit Flag']
    for i,h in enumerate(headers,1):
        style_cell(ws.cell(19,i,h),bg=ORANGE,bold=True,border=True,align='center')
    ...
    for r,sid in enumerate(scenarios,20):
        ws.cell(r,1,sid)
        style_cell(ws.cell(r,1),bg=LIGHT_BLUE,color=LINK_GREEN,bold=True,border=True,wrap=True)
        for c,src,fmt in [(2,'C',FMT_PCT),(3,'D',FMT_PCT),(4,'F',FMT_PCT2),(5,'P',FMT_MULT)]:
            formula(ws.cell(r,c),f'=INDEX(\'17_Scenarios_Sensitivity\'!${src}$2:${src}$14,MATCH($A{r},\'17_Scenarios_Sensitivity\'!$A$2:$A$14,0))',fmt,bg=YELLOW if c<5 else CYAN,bold=c==5)
        ws.cell(r,6,f'=IF(E{r}<1.20,"BREACH","PASS")')
        style_cell(ws.cell(r,6),bg=WHITE,bold=True,border=True,align='center')
        add_status_rules(ws,f'F{r}',f'F{r}')
    ...
    for c in range(2,7):
        ws.column_dimensions[ws.cell(1,c).column_letter].width=16
    for c in range(8,15):
        ws.column_dimensions[ws.cell(1,c).column_letter].width=13
    configure(ws,'A6')
...
def build_ic(wb):
    ...
    for r,(name,fx,fmt) in enumerate(info,7):
        label(ws.cell(r,1),name)
        formula(ws.cell(r,4),fx,fmt,bg=CYAN,bold=True,align='left' if fmt=='@' else 'right')
    ...
    for r,(name,col) in enumerate(dec,15):
        ...
        formula(ws.cell(r,4),project_formula('19_IC_Bankability',col),align='left',bold=name in {'Sponsor Status','Lender Status','Recommended Action','Final Classification','Formula Check'})
        if name in {'Sponsor Status','Lender Status','Final Classification','Formula Check'}:
            add_status_rules(ws,f'D{r}:N{r}',f'D{r}')
    ...
    for r,(name,fx,fmt) in enumerate(metrics,28):
        label(ws.cell(r,1),name,bg=CYAN)
        formula(ws.cell(r,4),fx,fmt,bg=CYAN,bold=True)
    ws.column_dimensions['A'].width=34
    for c in range(4,15):
...
def build_checks(wb):
    ...
    for i,h in enumerate(headers,1):
        style_cell(ws.cell(6,i,h),bg=ORANGE,bold=True,border=True,align='center')
    checks=[
    ...
    for r,(name,status,actual,purpose) in enumerate(checks,7):
        ws.cell(r,1,name)
        style_cell(ws.cell(r,1),bg=LIGHT_BLUE,color=NAVY,bold=True,border=True,wrap=True)
        formula(ws.cell(r,2),status,bg=WHITE,color=BLACK,bold=True,align='center')
        add_status_rules(ws,f'B{r}',f'B{r}')
        formula(ws.cell(r,3),actual,FMT_BN if r!=11 else FMT_MULT,bg=WHITE,color=BLACK)
        ws.cell(r,4,purpose)
        style_cell(ws.cell(r,4),bg=GREY,border=True,wrap=True)
    section(ws,14,'SOURCE MODEL VALIDATION STATUS',8)
    ws['A15']='Native Workbook QA'
    ws['B15']='See 21_QA_Audit and validation outputs'
    ws.merge_cells('B15:H15')
    style_cell(ws['A15'],bg=LIGHT_BLUE,color=NAVY,bold=True,border=True)
    style_cell(ws['B15'],bg=WHITE,color=LINK_GREEN,bold=True,border=True)
    ws.column_dimensions['A'].width=36
    ws.column_dimensions['B'].width=16
