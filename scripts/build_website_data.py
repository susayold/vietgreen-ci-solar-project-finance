...
overview = {
    ...
    "scenarioSummary": [
        {"label": "Base sponsor", "projectNPV": round_num(bvnd(base_scenario["project_npv_vnd"])), "equityNPV": round_num(bvnd(base_scenario["equity_npv_vnd"])), "dscr": round_num(base_scenario["min_dscr"], 3), "status": base_scenario["status"]},
        {"label": "P90 energy", "projectNPV": round_num(bvnd(scenario_by_name["P90_ENERGY"]["project_npv_vnd"])), "equityNPV": round_num(bvnd(scenario_by_name["P90_ENERGY"]["equity_npv_vnd"])), "dscr": round_num(scenario_by_name["P90_ENERGY"]["min_dscr"], 3), "status": scenario_by_name["P90_ENERGY"]["status"]},
        {"label": "Combined downside", "projectNPV": round_num(bvnd(scenario_by_name["COMBINED_DOWNSIDE"]["project_npv_vnd"])), "equityNPV": round_num(bvnd(scenario_by_name["COMBINED_DOWNSIDE"]["equity_npv_vnd"])), "dscr": round_num(scenario_by_name["COMBINED_DOWNSIDE"]["min_dscr"], 3), "status": scenario_by_name["COMBINED_DOWNSIDE"]["status"]},
    ],
    "built": [
...
economics = {
    ...
    "projectDetails": {
        project_id: {
            ...
            "returns": {"currentPPA": round_num(next(r for r in returns if r["project_id"] == project_id and r["case"] == "CURRENT_TERMS")["ppa_price_vnd_kwh"], 1), "negotiatedPPA": round_num(returns_lookup[(project_id, "NEGOTIATED_TERMS_HYPOTHETICAL")]["ppa_price_vnd_kwh"], 1), "capexFactor": round_num(returns_lookup[(project_id, "NEGOTIATED_TERMS_HYPOTHETICAL")]["capex_factor"], 2), "projectNPVBVND": round_num(bvnd(returns_lookup[(project_id, "NEGOTIATED_TERMS_HYPOTHETICAL")]["project_npv_vnd"])), "projectIRR": round_num(pct(returns_lookup[(project_id, "NEGOTIATED_TERMS_HYPOTHETICAL")]["project_irr"]), 2), "equityNPVBVND": round_num(bvnd(returns_lookup[(project_id, "NEGOTIATED_TERMS_HYPOTHETICAL")]["equity_npv_vnd"])), "equityIRR": round_num(pct(returns_lookup[(project_id, "NEGOTIATED_TERMS_HYPOTHETICAL")]["equity_irr"]), 2)},
...
portfolio = {
    ...
    "exposureLimits": [
        {"label": "Parent", "limitPct": 40, "currentPct": round_num(max(num(r["parent_equity_share_of_budget"]) for r in selected_exposure) * 100, 1), "status": "PASS"},
        {"label": "Industry", "limitPct": 60, "currentPct": round_num(max(num(r["industry_equity_share_of_budget"]) for r in selected_exposure) * 100, 1), "status": "PASS"},
        {"label": "Region", "limitPct": 70, "currentPct": round_num(max(num(r["region_equity_share_of_budget"]) for r in selected_exposure) * 100, 1), "status": "PASS"},
        {"label": "Total debt cap", "limitBVND": 80, "currentBVND": selected_total_debt, "status": "PASS"},
    ],
    "allocation": [
...
risk = {
    ...
    "tornado": [{"label": "P90 energy", "deltaBVND": delta("P90_ENERGY")}, {"label": "CAPEX overrun", "deltaBVND": delta("CAPEX_OVERRUN")}, {"label": "Interest rate shock", "deltaBVND": delta("INTEREST_RATE_SHOCK")}, {"label": "DSO delay", "deltaBVND": delta("DSO_DELAY")}, {"label": "COD delay", "deltaBVND": delta("COD_DELAY")}],
    ...
    "riskRegister": [{"risk": "PPA price risk", "impact": "High", "mitigation": "Diversify offtakers; indexation; price floor"}, {"risk": "CAPEX overrun", "impact": "High", "mitigation": "EPC fixed-price; contingency buffer"}, {"risk": "Interest-rate risk", "impact": "High", "mitigation": "Fixed rate, cap or hedge"}, {"risk": "Generation underperformance", "impact": "Medium", "mitigation": "Quality equipment and performance guarantees"}, {"risk": "COD delay", "impact": "Medium", "mitigation": "Milestone incentives and liquidated damages"}],
...
evidence = {
    ...
    "methodologies": [{"label": "Energy yield", "path": "docs/ENERGY_YIELD_METHODOLOGY.md"}, {"label": "PPA pricing", "path": "docs/PPA_PRICING_METHODOLOGY.md"}, {"label": "CFADS & tax", "path": "docs/CFADS_TAX_METHODOLOGY.md"}, {"label": "Debt sculpting", "path": "docs/DEBT_SCULPTING_METHODOLOGY.md"}, {"label": "FX financing", "path": "docs/FX_FINANCING_METHODOLOGY.md"}, {"label": "Portfolio", "path": "docs/PORTFOLIO_METHODOLOGY.md"}, {"label": "Model audit", "path": "docs/MODEL_AUDIT_METHODOLOGY.md"}],
