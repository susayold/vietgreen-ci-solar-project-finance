...
def capacity_constraints(cfads, rate, dscr, llcr, plcr, leverage_cap, capex, project_life_cfads=None):
    ...
    pv_llcr=discounted_value(loan_life,rate)/float(llcr) if llcr else float("inf")
    pv_plcr=discounted_value(project_life,rate)/float(plcr) if plcr else float("inf")
