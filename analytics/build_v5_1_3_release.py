...
def _workbook(root, model):
    ...
    for sheet_name, sheet_path in [("03_Physical_QA","validation/V5_1_2_PHYSICAL_QA.csv"),("04_Resolved_Input_View","outputs/v5_1_3_model_input_view.csv")]:
...
def build(root=ROOT):
    ...
    _write(root/"artifacts/v5_1_3_surfaces/content_contract.json",json.dumps({"release_version":"5.1.2","authoritative_economics":"outputs/v5_1_3_project_economics.csv","authoritative_website_data":["website/data/shared-summary.json","website/data/release-meta.json","website/data/projects.json","website/data/frontier.json","website/data/risk.json","website/data/evidence.json","website/data/scenarios.json"],"claim_boundary":summary["claimBoundary"],"derived_from":"analytics/build_v5_1_3_release.py","remote_only":True},indent=2)+"\n")
    ...
    _write(root/"validation/V5_1_2_RED_TEAM_REPORT.md","""# V5.1.3 Red-Team Closure Report
...
""")
    ...
    _csv(root/"validation/V5_1_2_REMEDIATION_REGISTER.csv",[{"finding":a,"resolution":b,"resolved_commit":os.getenv("GITHUB_SHA","CI_SEALED_EXACT_HEAD"),"resolved_run":os.getenv("GITHUB_RUN_ID","CI_SEALED_RUNTIME_METADATA"),"resolved_artifact":"PENDING_PRIMARY_ARTIFACT_ID","verification_test":"V5.1.3 contract / CI readback","status":c} for a,b,c in findings],["finding","resolution","resolved_commit","resolved_run","resolved_artifact","verification_test","status"])
    static_contract={"release_version":"5.1.2","release_tag":"v5.1.3-recruiter-final","release_status":"FINAL_RECRUITER_RELEASE","candidate_history_count":54,"selected_project_count":20,"raw_observation_count":441,"ppa_mode":"FRONTIER_ONLY","reference_case":"REFERENCE_CASE_NOT_ACTUAL_PPA","decision_boundary":"INDETERMINATE_MISSING_COMMERCIAL_DATA","transaction_evidence_status":"OPEN","bankable_transaction_ready":False,"lender_approval_ready":False,"ic_approval_ready":False,"recruiter_ready":True,"runtime_manifest_authority":"CI_ARTIFACT","remote_only":True}
