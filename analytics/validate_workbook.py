...
This is a package/XML integrity check executed on the ephemeral GitHub Actions
runner. It is not a substitute for independent engineering, lender, legal or
desktop-Excel review.
"""
...
EXPECTED_SHEETS = [
    "00_Control",
    "01_Assumptions",
    "02_Evidence_Regulatory",
    "03_Project_Pipeline",
    "04_Offtakers_Credit_Site",
    "05_Solar_Energy",
    "06_Load_PPA",
    "07_CAPEX_Construction",
    "08_OPEX",
    "09_Project_CF_CFADS",
    "10_Portfolio_CFADS",
    "11_Debt_Terms",
    "12_Debt_Sculpting",
    "13_Reserves_Waterfall",
    "14_Coverage",
    "15_Returns_Discount",
    "16_FX_Financing",
    "17_Scenarios_Sensitivity",
    "18_Portfolio",
    "19_IC_Bankability",
    "20_External_Validation",
    "21_QA_Audit",
]
...
def run() -> dict[str, int]:
    ...
    if not WORKBOOK.exists():
        ...
        add(
            ...
            "True",
            "PASS",
            impact="Native workbook is present in the remote checkout.",
        )
        add(
            "WB-002",
            "zip_magic",
            "PK",
            raw[:2].decode("latin-1", errors="replace"),
            "PASS" if raw[:2] == b"PK" else "FAIL",
            impact="Confirms the binary is an OOXML ZIP package.",
        )
        try:
            with zipfile.ZipFile(WORKBOOK) as zf:
                ...
                add(
                    ...
                    impact="Detects corrupt ZIP members.",
                )
                ...
                add(
                    "WB-005",
                    "sheet_count",
                    str(len(EXPECTED_SHEETS)),
                    str(len(sheet_names)),
                    "PASS" if len(sheet_names) == len(EXPECTED_SHEETS) else "FAIL",
                    impact="Checks the required 22-sheet architecture.",
                )
                add(
                    "WB-006",
                    "sheet_contract",
                    json.dumps(EXPECTED_SHEETS, ensure_ascii=False),
                    json.dumps(sheet_names, ensure_ascii=False),
                    "PASS" if sheet_names == EXPECTED_SHEETS else "FAIL",
                    impact="Prevents silent sheet renaming or reordering.",
                )
                ...
                for index in range(1, len(EXPECTED_SHEETS) + 1):
                    ...
                    if present:
                        try:
                            ...
                            parseable = False
                    add(
                        f"WB-SHEET-{index:02d}",
                        f"worksheet_{index:02d}_xml",
                        "Present and parseable",
                        "Present and parseable" if present and parseable else "Missing or invalid",
                        "PASS" if present and parseable else "FAIL",
                ...
                for check_id, token, impact in [
                    (
                        "WB-030",
                        "VietGreen_CI_Solar_Project_Finance",
                        "Control sheet must identify the project.",
                    ),
                    (
                        "WB-031",
                        "PASS_WITH_LIMITATIONS",
                        "Control sheet must expose the release boundary.",
                    ),
                    (
                        "WB-032",
                        "13 / 13 PASS",
                        "Control sheet must expose data-quality status.",
                    ),
                ]:
                    add(
                        check_id,
                        "control_metadata_token",
                        token,
                        "Present" if token in control_xml else "Missing",
                        "PASS" if token in control_xml else "FAIL",
                        impact=impact,
                    )
            ...
            add(
                ...
                type(exc).__name__,
                "FAIL",
                impact="The workbook cannot be trusted as a readable OOXML package.",
            )
    ...
    report_lines = [
        ...
        "",
        "Remote structural validation of the committed native workbook.",
        "",
        f"- Checks run: {len(checks)}",
        f"- Passed: {len(checks) - failures}",
        f"- Failed: {failures}",
