...
def build():
    ...
    fx_rows = [
        ...
        {"qa_id": "FX-006", "requirement": "Hedge changes FX sensitivity and records an explicit fee", "status": "PASS" if all(row["min_dscr_usd_4pct_hedged"] >= row["min_dscr_usd_4pct"] - 1e-6 for row in fx_output) else "PARTIAL", "metric": "fee=%0.2f%%" % (USD_HEDGE_FEE * 100), "evidence_path": "outputs/fx_funding_comparison_v4.csv"},
