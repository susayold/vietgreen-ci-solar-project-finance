...
function renderEconomics(data, projectId = data.defaultProjectId) {
  ...
  const rows = [
    ["PPA price (VND/kWh)", n(returns.currentPPA, 1), n(returns.negotiatedPPA, 1), `<span class="positive">${pct((returns.negotiatedPPA / returns.currentPPA - 1), 1)}</span>`],
...
function renderPortfolio(data) {
  const rows = data.selectedProjects.map((row) => [esc(row.projectId), esc(row.name), esc(row.region), esc(row.industry), `<span class="num">${bvnd(row.equityRequiredBVND)}</span>`, `<span class="num">${bvnd(row.debtBVND)}</span>`, `<span class="num positive">${bvnd(row.equityNPVBVND)}</span>`, `<span class="num">${n(row.equityIRR, 2)}%</span>`, `<span class="num">${n(row.minDSCR, 2)}x</span>`]);
  return `<div class="page">${pageHero(data, "portfolio", {actions: `<a class="button button-primary" href="#/risk">Stress portfolio →</a>`})}${sharedKpis(data.shared)}${section("Portfolio funnel", "Value-positive remediation is not the same as final selection.", `<div class="grid-4">${data.funnel.map((step, index) => `<div class="panel panel-pad"><div class="section-kicker">0${index + 1}</div><div class="kpi-value">${n(step.value, 0)}</div><div class="muted">${esc(step.label)}</div></div>`).join("")}</div>`, "01 · SCREENING")}${section("Selected portfolio (four projects)", "The final exposure-constrained case is sourced from portfolio_exposure_v4.csv.", panel("Selected project table", table(["ID", "Project", "Region", "Industry", "Equity", "Debt", "Equity NPV", "IRR", "DSCR"], rows), "Negotiated hypothetical"), "02 · SELECTION")}${section("Exposure limits and capital allocation", "Concentration limits are displayed beside the selected case.", `<div class="grid-2">${panel("Exposure compliance", table(["Limit", "Policy", "Current", "Status"], data.exposureLimits.map((row) => [esc(row.label), row.limitPct ? `${n(row.limitPct, 0)}%` : `≤ ${n(row.limitBVND, 0)} BVND`, row.currentPct ? `${n(row.currentPct, 1)}%` : `${bvnd(row.currentBVND)}`, status(row.status)])), "All PASS")}${panel("Equity allocation", barChart(data.allocation, {format: (value) => bvnd(value)}), "Selected equity required")}</div>`, "03 · CONTROLS")}${section("Why other projects did not make it", "Current-terms rejection reasons remain visible instead of being hidden behind a score.", `<div class="grid-2">${panel("Rejection reason count", barChart(data.rejectionReasons.map((row) => ({label: row.label, value: row.count})), {format: (value) => n(value, 0)}), "Current terms")}${panel("Standalone vs pooled", table(["Metric", "Standalone", "Pooled"], data.pooling.map((row) => [esc(row.label), `<span class="num">${typeof row.standalone === "number" ? n(row.standalone, 3) : esc(row.standalone)}</span>`, `<span class="num">${typeof row.pooled === "number" ? n(row.pooled, 3) : esc(row.pooled)}</span>`])), "Pooling output")}</div>`, "04 · ALLOCATION")}${sourceList(data.sources)}</div>`;
}

function renderRisk(data) {
  const scenarioRows = data.scenarios.map((row) => [esc(row.scenario), esc(row.note), `<span class="num ${cls(row.projectNPVBVND)}">${bvnd(row.projectNPVBVND)}</span>`, `<span class="num ${cls(row.equityNPVBVND)}">${bvnd(row.equityNPVBVND)}</span>`, `<span class="num">${n(row.minDSCR, 2)}x</span>`, status(row.status)]);
  return `<div class="page">${pageHero(data, "risk", {actions: `<a class="button button-primary" href="#/evidence">Review open gates →</a>`, aside: `<div class="decision-banner"><small>CURRENT TERMS DECISION</small><strong>NO DEPLOYMENT</strong><span>${esc(data.currentDecision.text)}</span></div>`})}${sharedKpis(data.shared)}${section("Scenario matrix", "The selected portfolio is profitable in base, but the downside path is not bankable.", panel("Phase 2 scenario outputs", table(["Scenario", "Scenario note", "Project NPV", "Equity NPV", "Min DSCR", "Status"], scenarioRows), "V4 phase 2"), "01 · DOWNSIDE")}${section("Tornado sensitivity", "One-at-a-time scenario deltas versus base Equity NPV; negative bars indicate value destruction.", panel("Equity NPV sensitivity", barChart(data.tornado.map((row) => ({label: row.label, value: row.deltaBVND})), {format: (value) => bvnd(value)}), "BVND change"), "02 · DRIVERS")}${section("Fixed debt vs re-sized debt", "Debt re-sizing can restore headroom, but does not remove commercial or execution risk.", `<div class="grid-2">${panel("Comparison", table(["Metric", "Fixed debt", "Re-sized debt"], data.fixedVsResized.map((row) => [esc(row.label), `<span class="num">${typeof row.fixed === "number" ? n(row.fixed, 3) : esc(row.fixed)}</span>`, `<span class="num positive">${typeof row.resized === "number" ? n(row.resized, 3) : esc(row.resized)}</span>`])), "Illustrative remediation")}${panel("Stress summary", data.stressSummary.map((row) => `<div class="metric-line"><span>${esc(row.label)}</span><b class="negative">${bvnd(row.value)}</b></div>`).join(""), "Equity view")}</div>`, "03 · CREDIT RESPONSE")}${section("Top risk register", "Risks are paired with a mitigation owner action, not only a colour.", panel("Risk register", table(["Risk", "Impact", "Mitigation"], data.riskRegister.map((row) => [esc(row.risk), `<span class="warning">${esc(row.impact)}</span>`, esc(row.mitigation)])), "Screening view"), "04 · REGISTER")}${sourceList(data.sources)}</div>`;
}

function renderModel(data) {
  return `<div class="page">${pageHero(data, "model", {actions: `${gh("model/vietgreen_v4_formula_model.xlsx", "Open workbook")}<a class="button button-primary" href="#/evidence">Evidence & downloads →</a>`})}${kpis(data.shared, [{label: "Formula cells", value: n(data.qa.formulaCells, 0), meta: "Workbook QA"}, {label: "Formula errors", value: n(data.qa.formulaErrors, 0), meta: "After recalculation", tone: "positive"}, {label: "Excel ↔ Python", value: data.qa.excelPythonReconciliation, meta: "Reconciliation", tone: "positive"}, {label: "Final DoD", value: data.qa.finalDoD, meta: "All pass", tone: "positive"}, {label: "Red team", value: data.qa.redTeam, meta: "Sampled tests", tone: "positive"}, {label: "Release stage", value: data.qa.stageFlow, meta: "Gates"}])}${section("Model architecture", "The release is built as a deterministic chain from inputs to recruiter-safe communication.", `<div class="panel architecture">${data.architecture.map((node, index) => `<div class="arch-node">${esc(node)}</div>${index < data.architecture.length - 1 ? "<span class=\"arch-arrow\">→</span>" : ""}`).join("")}</div>`, "01 · PIPELINE")}${section("Workbook architecture", "A real workbook preview is included in the repository; this page makes the structure explicit.", `<div class="grid-2">${panel("V4 workbook sheets", `<div class="grid-3">${data.workbookSheets.map((sheet, index) => `<div class="callout"><b>0${index + 1}</b><br>${esc(sheet)}</div>`).join("")}</div>`, "Formula-driven")}${panel("Workbook preview", `<iframe class="embed" title="VietGreen V4 workbook preview" src="model_preview/index.html"></iframe><p class="muted">The preview is a recruiter-safe communication layer; the source workbook remains linked on GitHub.</p>`, "Inspect")}</div>`, "02 · WORKBOOK")}${section("Reproducibility and validation", "Build identifiers let a reviewer reproduce the release without treating the website as the model.", `<div class="grid-2">${panel("Reproducibility", `<div class="metric-list"><div class="metric-line"><span>Locked seed</span><b>${n(data.reproducibility.seed, 0)}</b></div><div class="metric-line"><span>Release ID</span><b>${esc(data.reproducibility.releaseId)}</b></div><div class="metric-line"><span>Workbook SHA-256</span><b class="muted" style="font-size:10px;word-break:break-all">${esc(data.reproducibility.workbookHash)}</b></div><div class="metric-line"><span>Match confirmed</span>${status(String(data.reproducibility.matchConfirmed).toUpperCase())}</div></div>`, "Manifest")}${panel("Validation log", table(["Check", "Status", "Detail"], data.validationLog.map((row) => [esc(row.check), status(row.status), esc(row.detail)])), "Release evidence")}</div>`, "03 · QA")}${section("Red-team tests", "Sampled stresses are named so a reviewer can find the corresponding validation artifacts.", `<div class="grid-3">${data.redTeam.map((item) => `<div class="evidence-card"><span class="evidence-icon">✓</span><div><h3>${esc(item)}</h3><p class="positive">PASS</p></div></div>`).join("")}</div>`, "04 · RED TEAM")}${sourceList(data.sources)}</div>`;
}
