"""V5.1.3 economics engine: observed facts, explicit overlay assumptions, and auditable PF outputs."""
...
def _rate(x, default=0.0):
    """These overlay fields are percentage points, including values <= 1."""
    return _num(x, default) / 100.0
...
def _irr(values):
    ...
    if sum(a != b for a,b in zip(signs,signs[1:])) != 1: return ""
    lo,hi=-.99,10.0
    if _npv(values,lo) * _npv(values,hi) > 0: return ""
    for _ in range(100):
...
def _project_inputs(project, a, physical):
    ...
    return {
        ...
        "observed_generation_kwh":_num(physical.get("observed_generation_kwh")),
        "physical_status":physical["physical_status"], "base_generation_origin":physical["base_generation_origin"],
        "engineering_review_required":physical["engineering_review_required"], "input_ready_status":physical["model_input_status"],
...
def _debt_metrics(p, cfads):
    ...
    debt,binding,constraints=capacity_constraints(loan,p["debt_rate"],p["debt_sculpting_dscr"],p["llcr_min"],p["plcr_min"],p["leverage_cap"],p["capex_local"],project_life_cfads=project,llcr_rate=p["llcr_discount_rate"],plcr_rate=p["plcr_discount_rate"])
...
def _apply_contractual_schedule(
    base_schedule: List[Dict],
    stressed_cfads: List[float],
    base_rate: float,
    scenario_rate: float,
    debt_rate_type: str,
) -> List[Dict]:
    """Preserve contractual opening/principal/closing; reprice only floating interest."""
    _ = base_rate
    rows = []
    for i, base in enumerate(base_schedule):
        opening = float(base["opening"])
        principal = float(base["principal"])
        closing = float(base["closing"])
        if debt_rate_type == "FLOATING_REFERENCE":
            interest = opening * float(scenario_rate)
        else:
            interest = float(base["interest"])
        service = principal + interest
        cash = float(stressed_cfads[i]) if i < len(stressed_cfads) else 0.0
        rows.append({
            "opening": opening,
            "interest": interest,
            "principal": principal,
...
def _contractual_schedule_for_mode(
    mode: str,
    base_schedule: List[Dict],
    stressed_cfads: List[float],
    base_rate: float,
    ...
    if mode in ("FIXED_CONTRACTUAL_SCHEDULE", "NO_NEW_DEBT"):
        return _apply_contractual_schedule(
            base_schedule, stressed_cfads, base_rate, scenario_rate, debt_rate_type
        )
    raise ValueError(f"Unsupported debt mode for contractual schedule: {mode}")


def _schedule_signature(schedule: List[Dict], field: str) -> str:
    return "|".join(
        f"{float(row[field]):.8f}"
        for row in schedule
    )
...
def run(root: str|Path, output_dir: str|Path) -> Dict[str,List[Dict]]:
    ...
    for project in projects:
        ...
        physical=physical_rows[project["project_id"]]
        if physical["model_input_status"] != "READY_FOR_ECONOMICS": continue
        p=_project_inputs(project,overlays[project["project_id"]],physical)
        ...
        def sponsor_fn(x):
            ...
            service=[z["debt_service"] for z in sched]
        ...
        row={"project_id":project["project_id"],"project_name":project["project_name"],"country":project["country"],"currency":p["currency"],
          "input_origin":"OBSERVED_FACTS_PLUS_PHYSICAL_RESOLUTION_PLUS_EXPLICIT_OVERLAY","installed_capacity_kwp_observed":project.get("installed_capacity_kwp_observed",""),
          "observed_generation_kwh":physical.get("observed_generation_kwh",""),"physical_status":p["physical_status"],"generation_p50_origin":p["base_generation_origin"],
          "engineering_review_required":p["engineering_review_required"],"input_ready_status":p["input_ready_status"],
        ...
        for sid in scenario_ids:
            ...
            if mode=="RESIZED_DEBT":
            ...
            elif mode in ("FIXED_CONTRACTUAL_SCHEDULE", "NO_NEW_DEBT"):
                debt_s=debt
                sch_s=_contractual_schedule_for_mode(mode,sched,scf,p["debt_rate"],rate,p["debt_rate_type"])
                ...
                raise ValueError(f"Unsupported debt mode: {mode}")
            scenario_rows.append({
              "project_id":project["project_id"],"scenario_id":sid,"debt_mode":mode,
              "schedule_preservation_required":str(mode in ("FIXED_CONTRACTUAL_SCHEDULE","NO_NEW_DEBT")).upper(),
              "rate_response":sp["rate_delta"],"cod_delay_years":sp["cod_delay_years"],
              "energy_factor":sp["energy_factor"],"capex_factor":sp["capex_factor"],
              "opex_factor":sp["opex_factor"],"collection_haircut":sp["collection_haircut"],
              "termination_year":sp["termination_year"],
              ...
              "base_debt_local":debt,"scenario_debt_local":debt_s,
              ...
              "opening_schedule_preserved":str(_schedule_signature(sched,"opening")==_schedule_signature(sch_s,"opening")).upper(),
              "base_principal_schedule_signature":_schedule_signature(sched,"principal"),
              "scenario_principal_schedule_signature":_schedule_signature(sch_s,"principal"),
              "principal_schedule_preserved":str(_schedule_signature(sched,"principal")==_schedule_signature(sch_s,"principal")).upper(),
              "base_closing_schedule_signature":_schedule_signature(sched,"closing"),
              "scenario_closing_schedule_signature":_schedule_signature(sch_s,"closing"),
              "closing_schedule_preserved":str(_schedule_signature(sched,"closing")==_schedule_signature(sch_s,"closing")).upper(),
              ...
              "interest_numeric_signature_changed":str(_schedule_signature(sched,"interest")!=_schedule_signature(sch_s,"interest")).upper(),
              ...
              "base_debt_schedule_preserved":str(mode in ("FIXED_CONTRACTUAL_SCHEDULE","NO_NEW_DEBT") and _schedule_signature(sched,"principal")==_schedule_signature(sch_s,"principal")).upper(),
