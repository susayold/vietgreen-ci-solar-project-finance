...
def discounted_value(values, rate):
    return sum(float(v)/(1+float(rate))**i for i,v in enumerate(values,1))

def coverage_ratio(cfads, debt_service):
    return min((float(c)/float(s) for c,s in zip(cfads,debt_service) if float(s)>0), default=0.0)

def capacity_constraints(cfads, rate, dscr, llcr, plcr, leverage_cap, capex, project_life_cfads=None, llcr_rate=None, plcr_rate=None):
    """Use loan-life CFADS for LLCR and project-life CFADS for PLCR."""
    loan_life=[float(x) for x in cfads]
    project_life=loan_life if project_life_cfads is None else [float(x) for x in project_life_cfads]
    dscr_capacity,_=backward_capacity(loan_life,rate,dscr)
    pv_llcr=discounted_value(loan_life,rate if llcr_rate is None else llcr_rate)/float(llcr) if llcr else float("inf")
    pv_plcr=discounted_value(project_life,rate if plcr_rate is None else plcr_rate)/float(plcr) if plcr else float("inf")
    lev=float(capex)*float(leverage_cap)
    choices={"DSCR":dscr_capacity,"LLCR":pv_llcr,"PLCR":pv_plcr,"LEVERAGE":lev}
    binding=min(choices,key=choices.get)
    return max(0.0, choices[binding]),binding,choices
